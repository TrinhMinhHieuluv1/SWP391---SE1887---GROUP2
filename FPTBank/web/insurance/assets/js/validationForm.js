...
Validator.maxLength = function (selector, max, message) {
    return {
        selector: selector,
        test: function (value) {
            return value.length <= max ? undefined : message || `Vui lòng nhập không quá ${max} ký tự`;
        }
    }
}
...
Validator.isLessThan = function (selector, maxValue, message) {
    return {
        selector: selector,
        test: function (value) {
            return parseFloat(value) < maxValue ? undefined : message || `Giá trị nhập vào phải nhỏ hơn ${maxValue}`;
        }
    }
}
