 ...
 * @author SCN
 */
public class RequireEmailToCustomer implements Filter {

    private static final boolean debug = true;
    private FilterConfig filterConfig = null;

    public RequireEmailToCustomer() {
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (session.getAttribute("account") != null && session.getAttribute("account") instanceof Customer) {
            Customer customer = (Customer)session.getAttribute("account");
            if (customer.getEmail() == null || customer.getEmail().isEmpty()){
                httpResponse.sendRedirect("register-email");
                return;
            }
        } 
        chain.doFilter(request, response);
